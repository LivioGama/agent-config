# Install agent-rules:// URL scheme handler for Windows

$ErrorActionPreference = "Stop"

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$RepoDir = Split-Path -Parent $ScriptDir
$HandlerScript = Join-Path $RepoDir "handle-deeplink.bat"
...
$RegistryPath = "HKCU:\Software\Classes\agent-rules"
if (-not (Test-Path $RegistryPath)) {
    New-Item -Path $RegistryPath -Force | Out-Null
}

Set-ItemProperty -Path $RegistryPath -Name "(Default)" -Value "URL:agent-rules" -Force
Set-ItemProperty -Path $RegistryPath -Name "URL Protocol" -Value "" -Force
Set-ItemProperty -Path $RegistryPath -Name "Content Type" -Value "application/x-agent-rules" -Force
...
Set-ItemProperty -Path $OpenPath -Name "(Default)" -Value "Open Agent Rules" -Force
...
Write-Host "Installed agent-rules:// handler for Windows"
Write-Host "Test with: start agent-rules://https://example.com/rule.md"